...
import { getPlatformPrices, type Price } from "@/actions/auth.actions";
...
export default function PricingPage() {
  ...
  useEffect(() => {
    const fetchPrices = async () => {
      try {
        ...
        setError(
          "Échec du chargement des plans tarifaires. Veuillez réessayer plus tard."
        );
      }
    };
  ...
  const handleSelectPlan = async (priceId: string) => {
    ...
    try {
      const selectedPlan = platformPrices.find(
        (plan) => plan.price === priceId
      );
